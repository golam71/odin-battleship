...
describe("Gameboard Class", () => {
  ...
  describe("getMissedAttacks() method ", () => {
    test("tracks missed attacks via board state", () => {
      const board = new Gameboard("Player");
      board.receiveAttack([1, 1]);
      board.receiveAttack([2, 2]);

      const misses = board.getMissedAttacks();
  ...
  describe("allShipsSunk() method", () => {
    test("returns false when ships exist but aren't sunk", () => {
      gameboard.placeShip(ship, [0, 0], "horizontal");
      expect(gameboard.allShipsSunk()).toBe(false);
    });

    test("returns false when ships are partially hit", () => {
      gameboard.placeShip(ship, [0, 0], "horizontal");
